...
(defun my/kohler-db ()
  ...
  (cl-flet ((orcl (ip port sid)
    ...
    (let ((atgdev   (orcl "10.94.132.146" "9101" "atgdev"))
	  ...
	  (local    (orcl "127.0.0.1"     "1521" "orcl")))

      ;; Define our connections as an alist
      (let ((cs `(("local"        . ("sys"                     "Oradoc_db1"         ,local    "as sysdba"))
		  ("localcata"    . ("cataloga"                "cataloga"           ,local    ()))

		  ("devcore"      . ("core_dev"                "K0hl3r_4u"          ,atgdev   ()))
	...
	(destructuring-bind (cname sql-user sql-password sql-database sql-oracle-options)
